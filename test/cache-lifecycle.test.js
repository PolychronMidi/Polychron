...
describe('composer cache lifecycle', () => {
  it('emits no composer:cache:miss in a short run', async () => {
    ...
    // Retry loop: avoid long blocking when another play has the lock by retrying briefly
    const MAX_RETRIES = 120;
    let res;
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      const { LONG_CHILD_PROC_TIMEOUT } = require('./test-timeouts');
      res = spawnSync(process.execPath, [path.join('scripts','play-guard.js')], { env, stdio: 'pipe', timeout: LONG_CHILD_PROC_TIMEOUT });
