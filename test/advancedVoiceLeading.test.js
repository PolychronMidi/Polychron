// Unit tests for VoiceLeadingComposer integration with MotifSpreader
require('../src/composers/VoiceLeadingComposer');
require('../src/composers/motifSpreader');

describe('VoiceLeadingComposer.selectNoteWithLeading', () => {
  it('prefers previous note when available', () => {
    const c = new VoiceLeadingComposer('major','C',0.9);
  ...
  it('influences MotifSpreader picks when set as layer.measureComposer', () => {
    const layer = { beatMotifs: { 0: [{ note: 60 }, { note: 67 }, { note: 72 }] }, measureComposer: new VoiceLeadingComposer('major','C',0.9) };
