...
## Decision
Phase 2: P1 + P2 approved + implemented + verified. P1: recordPolicyDeny logs the firing policy; i/why mode=debt prints `policy_dead_weight: N/M 0-fire (REVIEW, not auto-retire)` (live: 16/21, mostly rare critical guards -- exactly why it's REVIEW not retire). P2: scripts/immune_metabolize.js (node bridge, keeps the JS metabolism schema authoritative) classifies recurring error-log noise and dedup-appends durable context_metabolism facts; universal_pulse_tick.py spawns it throttled (~10min, fire-and-forget, off hot path); mode=debt prints `immune_memory: N classes metabolized`. Evidence: coherence_substrate 26/26, regression batch 36/36, ownership audit 41/41. Skips held (P4 field-widen, UPS p95 live attribution). Post-impl conferral with Agent2 available on request.
