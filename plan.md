...
## Decision
Phase 2: P1 + P2 + P3 (honesty fix) approved + implemented + verified. P1: recordPolicyDeny logs the firing policy; mode=debt prints `policy_dead_weight: N/M 0-fire (REVIEW, not auto-retire)` + coverage caveat (live: 16/21). P2: immune classify->metabolize off the hot path, now self-honest (upsert/refresh + recency-decay-out). Evidence: coherence_substrate 26/26, regression 20/20+36/36, ownership 41/41. Skips held. Agent2 pre-committed "after this pass: phase done" -- re-conferring to confirm clean close.
