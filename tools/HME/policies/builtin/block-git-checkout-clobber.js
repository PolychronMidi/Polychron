...
const REASON =
  'BLOCKED: this `git checkout` pattern can clobber the working tree (including freshly-popped stashes). For prior-state inspection use `git show <ref> -- <path>` (read-only stdout) or `git worktree add /tmp/x <ref>` (isolated). local-path-ok See doc/self-coherence.md or doc/templates/AGENTS.md.';
const TODO_REASON =
