...
INPUT=$(cat)
_DECISION=$(printf '%s' "$INPUT" | node -e "const fs=require('fs'); const {preWriteCheck,toHookResponse}=require(process.env.PROJECT_ROOT + '/tools/HME/proxy/pre_write_check'); (async()=>{const d=await preWriteCheck(fs.readFileSync(0,'utf8')); const out=toHookResponse(d); if(out) process.stdout.write(out);})().catch(e=>{process.stderr.write(e.stack||String(e)); process.exit(1);});" 2>/tmp/hme-prewrite-write.err)
_RC=$?
if [ "$_RC" -ne 0 ]; then _emit_block "BLOCKED: central pre-write check failed before writing. $(tail -c 500 /tmp/hme-prewrite-write.err 2>/dev/null)"; exit 2; fi
if [ -n "$_DECISION" ]; then
  printf '%s\n' "$_DECISION"
