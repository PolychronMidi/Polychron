...
if [ "${HME_PROXY_ENABLED}" = "1" ]; then
  ...
  if [ "$_ss_probe_status" != "ok" ]; then
    # User-facing stderr surface is the right channel; hme-errors.log entry
    # was self-flagellating noise (alerting that the session hook checked
...
# store.load() applies due 2_/4f_ timer flips; archive when nothing is in
# progress and >=1 item is 5_, carrying the non-5_ items into the next set.
store.maybe_archive()
