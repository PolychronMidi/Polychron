...
_ac_do_commit() {
  ...
  sleep 1
  # shellcheck disable=SC2086
  if git -C "$_AC_ROOT" commit -a -m "${commit_msg}-retry" --quiet $_commit_flags >"$_ac_err_buf" 2>&1; then
    _ac_success
    rm -f "$_ac_err_buf" 2>/dev/null
    exec 9>&-
    return 0
...
_ac_queue_drain_once() {
  ...
  if [ -d "$batch" ]; then
    # silent-ok: a missing caller name only affects the commit-message label.
