...
_hme_check_errors_inline() {
  ...
  # Self-origin source tags: shared with Stop lifesaver.
  local _SELF_TAG_RE
  _SELF_TAG_RE="$(_hme_self_tag_re)"
  ...
  # PROJECT_ROOT is required at file top and is authoritative; the resolver is
  # invoked via `node` (executable bit irrelevant), so no BASH_SOURCE-relative
  # fallback -- that would resolve into the plugin cache when sourced from there.
  ...
  if [ -f "$_FILTER_RESOLVED" ]; then
    AGENT_ERRORS=$(printf '%s\n' "$_REMAINING" | /usr/bin/grep -vE "$_OBS_RE" | /usr/bin/grep -v '^$' | PROJECT_ROOT="$PROJECT" node "$_FILTER_RESOLVED" || true)
  else
