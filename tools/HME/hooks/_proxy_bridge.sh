...
if [ "$EVENT" = "Stop" ] && [ -n "$_PB_ROOT" ]; then
  ...
  if [ -f "$_PB_TSCRIPT" ]; then
    ...
    printf '%s\n' "$_PB_TSCRIPT" > "$_PB_ROOT/tmp/hme-transcript-path.txt.tmp" 2>/dev/null && mv "$_PB_ROOT/tmp/hme-transcript-path.txt.tmp" "$_PB_ROOT/tmp/hme-transcript-path.txt" 2>/dev/null
  fi
fi
...
# rationale: 500ms retry covers proxy-restart warmup window vs lifecycle route
if [ -z "$RESP" ]; then
  sleep 0.5
  RESP=$(curl -sf --max-time 60 -X POST \
    -H 'Content-Type: application/json' \
