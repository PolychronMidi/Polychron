...
_SV_WORKER_PORT="$(_hme_service_port worker 2>/dev/null || printf '%s' "${HME_WORKER_PORT:-9098}")"  # silent-ok: registry helper absent -> fall back to HME_WORKER_PORT env var (default 9098)
_SV_WORKER_URL="$(_hme_service_url worker 2>/dev/null || printf 'http://127.0.0.1:%s/health' "$_SV_WORKER_PORT")"  # silent-ok: registry helper absent -> construct the canonical localhost worker health URL from the resolved port
...
_sv_loop() {
  ...
  if command -v flock >/dev/null 2>&1; then
    if ! flock -n 200 2>/dev/null; then  # silent-ok: lock denial logs/exits; no duplicate supervisor.
