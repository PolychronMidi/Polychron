...
test('watchdog requires UserPromptSubmit to match its SessionStart id', () => {
  const root = sandbox('hme-watchdog-strict-sessionstart-');
  fresh(root);
  const wd = require('../../event_kernel/hook_watchdog');
  const tok = wd.begin(root, 'SessionStart', JSON.stringify({ session_id: 'sessionstart-id' }), { host: 'test' });
  wd.end(tok, { exit_code: 0 });
  const alert = wd.userPromptAlert(root, JSON.stringify({ session_id: 'prompt-id' }));
  assert.match(alert, /before successful SessionStart/);
  assert.match(fs.readFileSync(path.join(root, 'log/hme-errors.log'), 'utf8'), /prompt-id/);
  fs.rmSync(root, { recursive: true, force: true });
});
