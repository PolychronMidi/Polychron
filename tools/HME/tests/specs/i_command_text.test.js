...
test('normalizes i wrapper path variants to canonical command text', () => {
  ...
  assert.strictEqual(normalizeICommand('./tools/HME/i/review -- mode=forget'), 'i/review mode=forget');
  assert.strictEqual(normalizeICommand('cd tools/HME && ../../tools/HME/i/status mode=health'), 'i/status mode=health');
