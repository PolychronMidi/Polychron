...
test('ask-peer forks the driver with full tool access (no local disallowed-tools path)', () => {
  ...
  try {
    ...
    assert.deepEqual(args.slice(0, 4), ['-p', '--resume', '22222222-2222-4222-8222-222222222222', '--fork-session']);
...
test('ask-peer fails closed before appending on invalid driver SID and appends explicit peer-error on bad JSON', () => {
  ...
  try {
    ...
    assert.equal(fs.readdirSync(path.join(root, 'teams/runtime')).some((f) => f.startsWith(`raw-${'full'}.`)), false,
      'raw peer stdout is stream-capped; no unbounded full-output temp file is created');
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
