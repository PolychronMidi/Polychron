...
test('structured edit rejects copied display-redacted sentinel with context', () => {
  const root = sandbox('hme-edit-redacted-');
  const file = path.join(root, 'src', 'target.js');
  ...
  try {
    ...
    assert.match(res.stdout, /pass actual file text/);
...
test('structured edit safely normalizes unique trailing whitespace mismatch', () => {
  const root = sandbox('hme-edit-ws-');
  const file = path.join(root, 'src', 'target.js');
  fs.writeFileSync(file, 'const value = 1;   \n');
  try {
...
test('structured edit autocorrects safe whitespace mismatch', () => {
  const root = sandbox();
  const file = path.join(root, 'src', 'target.js');
  fs.writeFileSync(file, 'const value = 1;   \n');
  try {
...
test('structured edit rejects copied display-redaction sentinel', () => {
  const root = sandbox();
  const file = path.join(root, 'src', 'target.js');
  fs.writeFileSync(file, 'const value = 1;\n');
  try {
...
test('structured edit rejects copied display-redacted placeholders with current context', () => {
  const root = sandbox('hme-edit-redacted-');
  const file = path.join(root, 'src', 'target.js');
  fs.writeFileSync(file, 'const value = 1;\n');
  try {
...
test('structured edit auto-applies unique trailing-whitespace-normalized match', () => {
  const root = sandbox('hme-edit-ws-');
  const file = path.join(root, 'src', 'target.js');
  fs.writeFileSync(file, 'const value = 1;   \n');
  try {
...
test('structured edit rejects copied display-redaction sentinel with context', () => {
  const root = sandbox('hme-edit-redacted-');
  const file = path.join(root, 'src', 'target.js');
  fs.writeFileSync(file, 'const value = 1;\n');
  try {
    const sentinel = '<display-redacted: original was sent; do not reuse>';
    const res = runEdit(root, [`file=${file}`, `old=${sentinel}`, 'new=const value = 2;']);
    assert.notEqual(res.status, 0);
    assert.match(res.stdout, /old_string is display-redacted/);
    assert.match(res.stdout, /const value = 1/);
  } finally {
...
test('structured edit safely normalizes trailing whitespace near misses', () => {
  const root = sandbox('hme-edit-ws-');
  const file = path.join(root, 'src', 'target.js');
  fs.writeFileSync(file, 'const value = 1;   \n');
  try {
...
test('display-redacted structured Edit old_string fails with current context', () => {
  ...
  fs.writeFileSync(file, 'const value = 1;\n');
  try {
    const res = runEdit(root, [
      `file=${file}`,
      'old=<display-redacted: original was sent; do not reuse>',
      'new=const value = 2;\n',
    ]);
    assert.notEqual(res.status, 0);
    assert.match(res.stdout, /old_string is display-redacted/);
...
test('malformed structured edit path fails loud instead of empty success', () => {
  const root = sandbox('hme-edit-malformed-path-');
  try {
    const res = runEdit(root, ["file=<<'HME_CODEX_JSON',", 'old=anything', 'new=anything else']);
    const combined = `${res.stdout}\n${res.stderr}`;
    assert.notEqual(res.status, 0);
    assert.notEqual(combined.trim(), '');
    assert.match(combined, /invalid file_path|malformed Edit/);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});

test('structured edit rejects display-redacted old_string with current context', () => {
  const root = sandbox('hme-edit-redacted-old-');
  const file = path.join(root, 'src', 'target.js');
  fs.writeFileSync(file, 'const value = 1;\n');
  try {
    const redacted = '<display-redacted: original was sent; do not reuse>';
    const res = runEdit(root, [`file=${file}`, `old=${redacted}`, 'new=const value = 2;\n']);
    assert.notEqual(res.status, 0);
    assert.match(res.stdout, /old_string is display-redacted/);
    assert.match(res.stdout, /\[READ current context src\/target\.js:/);
    assert.match(res.stdout, /const value = 1/);
...
test('structured edit rejects heredoc-fragment file paths loudly', () => {
  const root = sandbox('hme-edit-malformed-path-');
  try {
    const res = runEdit(root, ["file=<<'HME_CODEX_JSON',", 'old=x', 'new=y']);
    const combined = `${res.stdout}${res.stderr}`;
