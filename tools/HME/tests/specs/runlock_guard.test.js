...
test('runlock: mv away', async () => { await _runDeny(`mv tmp/${LOCK} /tmp/elsewhere local-path-ok`); });
...
test('runlock: command without the lock string is allowed', async () => {
  ...
  await _runAllow(`rm /tmp/somefile.txt local-path-ok`);
});

test('runlock: deletion verb against UNRELATED file is allowed (no run.lock token)', async () => {
  await _runAllow('rm /tmp/cache.tmp local-path-ok');
});
