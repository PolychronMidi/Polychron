...
_SAMPLE = """# myth0s Mesh Coverage Map

## Policy-enforcement surfaces
| surface | file | status |
| --- | --- | --- |
| pre-write gate | a.js | reviewed |
| stop-chain | b.js | pending |

## State-mutation surfaces
| surface | file | status |
| --- | --- | --- |
| state registry | c.js | reviewed |
| tool-result | d.js | reviewed (clean audit) |
"""


class CoverageStatusTests(unittest.TestCase):
    def test_parses_sections_status_and_parenthetical(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(by["tool-result"]["status"], "reviewed")  # parenthetical stripped
