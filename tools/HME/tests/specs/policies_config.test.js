...
test('i/policies commands report stale renamed config keys', _withSandbox(async (sandbox) => {
  ...
  for (const cmd of ['list', 'all', 'summary', 'check']) {
    const r = spawnSync('node', [cli, cmd], {
      cwd: path.resolve(__dirname, '../../../..'),
      env: { ...process.env, PROJECT_ROOT: sandbox },
      encoding: 'utf8',
    });
    assert.notStrictEqual(r.status, 0, `${cmd} should fail on stale config`);
    assert.match(r.stderr, /stale disabled policy name 'block-comment-bloat' \(renamed to 'rewrite-comment-bloat'\)/, `${cmd} should report rename hint`);
  }
}));

test('i/policies check succeeds on valid config', _withSandbox(async (sandbox) => {
  _writeJson(path.join(sandbox, 'config', 'policies.json'), { disabled: ['block-mid-pipeline-write'] });
  const cli = path.resolve(__dirname, '..', '..', 'policies', 'cli.js');
  ...
  assert.match(r.stdout, /config names valid/);
}));
