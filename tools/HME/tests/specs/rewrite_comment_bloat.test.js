...
const policy = require('../../policies/builtin/rewrite-comment-bloat');
...
test('rewrite-comment-bloat: rewrites 3+ consecutive comment lines to keep first 2', async () => {
...
test('rewrite-comment-bloat: truncates comment line >= LONG_LINE chars', async () => {
...
test('rewrite-comment-bloat: allows tight content with annotation comments', async () => {
  const content = 'const x = 1;\n// silent-ok: ignore\n// silent-ok: ignore\n// silent-ok: ignore\nconst y = 2;\n';
  const r = await policy.fn(_ctx({ toolInput: { file_path: 'a.js', content } }));
  assert.strictEqual(r.decision, 'allow');
});

test('rewrite-comment-bloat: skips files with no known comment prefix', async () => {
