...
_correct_check "inline cd && i/status rewrites to absolute" \
  ...
  "$PROJECT_ROOT/tools/HME/i/status"

# tool_input.cwd to subdir
INPUT=$(jq -n --arg cwd "$PROJECT_ROOT/tools/HME/scripts" '{"tool_input":{"command":"i/status","cwd":$cwd}}')
OUT=$(echo "$INPUT" | bash tools/HME/hooks/pretooluse/pretooluse_bash.sh 2>&1)
if echo "$OUT" | grep -qF "$PROJECT_ROOT/tools/HME/i/status"; then
