...
def _drop_hallucinated_bullets(synthesis: str, changed_files: str,
                               diff_context: str, hunk_context: str) -> str:
    """Filter LLM synthesis: drop bullets whose cited file paths / backtick
    symbols do NOT appear in the source material (diff + hunks + changed files).

    Background: the adaptive 'What You May Have Missed' section is generated by
    an LLM. Despite explicit "only cite file:line from the diff" rules in the
    prompt, the model occasionally confabulates adjacent files or symbols
    (e.g. mentioning tools/HME/KB/todos.json when the diff only touched
    pretooluse_toolsearch.sh). This filter catches those post-hoc.

    Heuristic: a bullet is hallucinated if it contains at least one file-path
    OR backtick-quoted symbol, and NONE of those references appear verbatim
    (case-insensitive) in the source material. Bullets with zero references
    (pure prose advice) are kept as-is. Safe default on any parse error: return
    ...
    try:
        ...
        # Split on bullet boundaries: numbered (1. / **1.**), dashed (- / *),
        # or bold headers (**Title —**). Keep the boundary line with its body.
        ...
        for b in bullets:
            text = "\n".join(b)
            refs = path_re.findall(text) + [s for s in tick_re.findall(text)]
            refs = [r.strip().lower() for r in refs if r and r.strip()]
            if refs and not any(r in source_blob for r in refs):
                # Every reference is absent from source material → hallucinated.
                logger.info(
                    f"what_did_i_forget: dropped hallucinated bullet "
                    f"(refs={refs[:3]}, none matched diff)"
                )
                continue
            kept.extend(b)
        filtered = "\n".join(kept).strip()
        # If filter stripped everything, fall back to original rather than
        # emit an empty section (noisier UX, but preserves the signal).
    ...
    if not diff_context:
        ...
        try:
            ...
            if result:
                ...
                synthesis = _drop_hallucinated_bullets(synthesis, changed_files, diff_context, hunk_context)
            elif result is None:
