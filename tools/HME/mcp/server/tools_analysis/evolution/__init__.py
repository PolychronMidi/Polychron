"""HME evolution subpackage -- journal patterns, KB seeding, and public
re-export hub for submodule symbols. Lives in __init__.py so the subpackage
itself exposes the hub API directly, avoiding the sys.modules alias trick
that breaks importlib.reload().
...
Split into focused modules:
  ...
  evolution_selftest.py   -- hme_selftest, hme_hot_reload, RELOADABLE tables
  evolution_strategies.py -- contradict, stress, curate, forge, design
  evolution_evolve.py     -- unified evolve() tool
  evolution_invariants.py -- declarative invariant battery
  evolution_introspect.py -- tool-usage self-benchmark
  evolution_suggest.py    -- loc / pipeline / patterns suggestions
  evolution_next.py       -- next-bridge picker
"""
import json
import os
import re
...
def kb_seed(top_n: int = 15) -> str:
    ...
    try:
        ...
        logger.debug(f"unnamed-except evolution/__init__.py:{__name__}: {type(_err).__name__}: {_err}")
        kb_names = set()
