        ...
        with self._lock:
            ...
            for _sweep in range(5):
                ...
                for _ in range(20):
                    ...
                    except ProcessLookupError:  # silent-ok: process died between SIGTERM and SIGKILL; intended outcome already achieved
                        pass
                    except OSError as e:
...
# boot-recovery function removed R97: it existed to unstick embedders
# stranded on coder's GPU by the migration-based indexing-mode. Now that
# indexing-mode no longer migrates, there's no stranded state to recover.
# If embedders end up on the wrong GPU, it's a worker-boot bug (in
# rag_engines.py device selection) — fix it there, don't paper over it
# from the daemon.
...
def main():
    ...
    try:
        ...
        try:
            os.unlink(PID_FILE)
        except OSError:  # silent-ok: boot-abort PID cleanup after topology-assertion failure
            pass
        sys.exit(2)
    ...
    try:
        ...
        try:
            os.unlink(PID_FILE)
        except OSError:  # silent-ok: shutdown PID cleanup; file may have been cleaned by signal handler
            pass
