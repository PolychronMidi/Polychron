...
class GeneratedISurfaceVerifier(Verifier):
    ...
    def run(self) -> VerdictResult:
        ...
        dispatcher = os.path.join(_PROJECT, "tools", "HME", "scripts", "hme-i-dispatch.js")
        generator = os.path.join(_PROJECT, "tools", "HME", "scripts", "generate-i-shims.js")
        try:
        ...
        i_dir = os.path.join(_PROJECT, "tools", "HME", "i")
        files = sorted(f for f in os.listdir(i_dir) if os.path.isfile(os.path.join(i_dir, f)) and not f.startswith("."))
        if files != commands:
            issues.append(f"tools/HME/i files differ from registry: files={files}, registry={commands}")
        for name in commands:
            ...
            try:
                ...
                issues.append(f"tools/HME/i/{name} unreadable: {e}")
                continue
            if "generated by tools/HME/scripts/generate-i-shims.js" not in body or f'"{name}" "$@"' not in body:
                issues.append(f"tools/HME/i/{name} is not a generated shim")
