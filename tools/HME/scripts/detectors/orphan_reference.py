...
def _collect_deleted_files(events: list[dict], project_root: str) -> list[str]:
    skip_prefixes = (
        tempfile.gettempdir().rstrip(os.sep) + os.sep, "/var/tmp/",
...
def _refs_remaining(project_root: str, deleted_path: str) -> list[str]:
    ...
    skip = ('/polychron-references/', tempfile.gettempdir().rstrip(os.sep) + os.sep,  # '/tools/HME/runtime/', '/src/output/metrics/',
