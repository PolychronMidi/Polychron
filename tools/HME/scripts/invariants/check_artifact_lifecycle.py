...
def main() -> int:
    ...
    metric_schema_required = cfg.get("metric_schema_required")
    if not isinstance(metric_schema_required, list) or not metric_schema_required:
        findings.append("metric_schema_required must be a nonempty list of tracked metric paths")
    elif not all(isinstance(x, str) and x for x in metric_schema_required):
        findings.append("metric_schema_required must contain only nonempty strings")
    metric_freshness_required = cfg.get("metric_freshness_required")
    if not isinstance(metric_freshness_required, list) or not metric_freshness_required:
        findings.append("metric_freshness_required must be a nonempty list of tracked metric paths")
    elif not all(isinstance(x, str) and x for x in metric_freshness_required):
        findings.append("metric_freshness_required must contain only nonempty strings")
