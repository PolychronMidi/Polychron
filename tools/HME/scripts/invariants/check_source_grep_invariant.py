...
RULES = {
    ...
    "hme-py-no-hardcoded-home": [{"roots": ["tools/HME/service", "tools/HME/hooks", "tools/HME/scripts", "tools/HME/activity", "tools/HME/proxy"], "include": [".py", ".sh", ".js"], "pattern": r"/home/" r"jah", "exclude": r"__pycache__|\.log:|# path-ok"}],
    ...
    "no-hardcoded-metrics-path": [{"roots": ["."], "include": [".js", ".py", ".sh"], "pattern": r"['\"]metrics/|path\.join\([^)]*['\"]metrics['\"]|os\.path\.join\([^)]*['\"]metrics['\"]", "exclude": r"metrics-ok|src/output/metrics|src['\"],\s*['\"]output['\"],\s*['\"]metrics|tools/HME/runtime/metrics|tools['\"],\s*['\"]HME['\"],\s*['\"]runtime['\"],\s*['\"]metrics|scripts/migrate-metrics-path\.py|src/scripts/pipeline/main-pipeline\.js|src/scripts/pipeline/validators/check-root-only-dirs\.js|tools/HME/proxy/infra/hme_paths\.js|tools/HME/KB/|tools/HME/tests/", "skip_dirs": {"node_modules", ".git", "__pycache__", ".venv"}}],
