...
RULES = {
    ...
    "no-hardcoded-metrics-path": [{"roots": ["."], "include": [".js", ".py", ".sh"], "pattern": r"['\"]metrics/|path\.join\([^)]*['\"]metrics['\"]|os\.path\.join\([^)]*['\"]metrics['\"]", "exclude": r"# metrics-ok|src/output/metrics|src['\"],\s*['\"]output['\"],\s*['\"]metrics|tools/HME/runtime/metrics|tools['\"],\s*['\"]HME['\"],\s*['\"]runtime['\"],\s*['\"]metrics|scripts/migrate-metrics-path\.py|tools/HME/KB/|tools/HME/tests/", "skip_dirs": {"node_modules", ".git", "__pycache__", ".venv"}}],
