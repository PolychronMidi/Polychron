...
function dispatchWhy(args) {
  ...
  for (const a of args) {
    ...
    const m = /^mode=(.+)$/.exec(a);
    if (m && ['proof', 'debt', 'mesh', 'resolve', 'metabolize', 'coherence-field', 'proof-capsules', 'causal-braid', 'immune', 'policy-genome', 'freshness'].includes(m[1])) run('node', [path.join(ROOT, 'tools/HME/scripts/why-coherence.js'), ...args]);
