...
def main(argv):
    if len(argv) < 2:
        ...
        if ids:
            ...
            print("  (no efficacy report found at tools/HME/runtime/metrics/hme-invariant-efficacy.json -- run pipeline first)",
                  file=sys.stderr)
        return 2
    inv_id = argv[1]

    hist = _load("tools/HME/runtime/metrics/hme-invariant-history.json") or {}
    eff = _load("tools/HME/runtime/metrics/hme-invariant-efficacy.json") or {}
