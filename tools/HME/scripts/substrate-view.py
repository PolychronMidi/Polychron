...
def brief():
    na = _load("tools/HME/runtime/metrics/hme-next-actions.json") or {}
    con = _load("tools/HME/runtime/metrics/hme-consensus.json") or {}
    dr = _load("tools/HME/runtime/metrics/hme-legendary-drift.json") or {}
    eff = _load("tools/HME/runtime/metrics/hme-invariant-efficacy.json") or {}
    ...
    hci_trend = ""
    try:
        rows = _load_jsonl_tail("tools/HME/runtime/metrics/hme-arc-timeseries.jsonl", n=3)
...
def detail():
    ...
    pm = _load("tools/HME/runtime/metrics/hme-pattern-matches.json") or {}
    if pm.get("matches"):
...
def diff_view():
    rows = _load_jsonl_tail("tools/HME/runtime/metrics/hme-arc-timeseries.jsonl", n=5)
    if len(rows) < 2:
