...
const REASONS = {
  ...
  QUOTE_PROVENANCE:
    'QUOTE FABRICATION DETECTED: final text attributes a delimited quote to the user ("you said \'X\'", "your words \'X\'", "as you put it `X`") whose span appears in NO real user prompt this conversation. Putting words in the user\'s mouth and then acting on them is the antipattern this gate blocks -- there is no "(verified)" waiver because provenance is objective. The gate clears once you (a) REMOVE the fabricated quote, (b) replace it with the user\'s ACTUAL words (copy them verbatim from a real user message), or (c) if you cannot quote a source, say so plainly ("I cannot quote where that came from") and drop the claim. Honest paraphrase and quoting tool output are fine; inventing a user quote is not.',
  PSYCHO_STOP:
