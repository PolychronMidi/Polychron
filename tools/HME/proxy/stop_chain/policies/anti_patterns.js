...
const REASONS = {
  ...
  FABRICATION:
    'FABRICATION DETECTED: final text asserts a quantitative invariant about pipeline state ("held steady", "stayed constant", "unchanged across runs", "same as last", etc.) without the turn containing a verification disclosure marker. In a stochastic music generator every run-level metric is different; invariance is the claim that needs proof, not the default. Choose one and resume: (a) VERIFY the claim now via i/status or Read src/output/metrics or grep run-history, then annotate the claim with "(verified)" / "(confirmed)"; (b) REMOVE the fabricated claim from the response; (c) EXPLICITLY qualify it with "(unverified)" / "(assumed)" / "(did not check)". Silent fabrication to bridge reasoning gaps is the antipattern this gate exists to block.',
  PSYCHO_STOP:
