...
const REASONS = {
  ...
  SCOPE_STACKED:
    'SCOPE-STACKED ANTIPATTERN: New `- [ ]` items were added to doc/templates/SPEC.md this turn but zero `[ ]` -> `[x]` transitions happened. Translation: you enumerated MORE work than you DID. The SPEC accumulates faster than artifacts. Either (a) implement the highest-leverage new item right now and tick it, or (b) revert the SPEC additions if they were premature scope-stacking. Naming work is not equivalent to shipping work. If the work is actually done and there is nothing left, silence is the correct response -- do NOT write ceremony text to dodge this gate.',
  SCOPE_NOT_TRACKED:
    'SCOPE-NOT-TRACKED ANTIPATTERN: This turn made substantive Edit/Write/MultiEdit calls to non-SPEC files but zero SPEC items were ticked. Either (a) tick the SPEC item(s) the work corresponds to, or (b) update SPEC.md to reflect the new scope the work actually addressed. Work happening off declared scope is silent scope drift. If the work is actually done and there is nothing left, silence is the correct response -- do NOT write ceremony text to dodge this gate.',
  EVASION_INTENT:
  ...
  BOYSCOUT_LOC:
    'LOC LIMIT VIOLATION (CLAUDE.md): A file you Edited/Wrote this turn exceeds 350 LOC and is not in config/loc-ignore.txt. Rule: "Files MUST be <=350 LOC unless listed in loc-ignore.txt. Organize at logical boundaries." Either split the file, OR add to loc-ignore.txt with rationale. If the work is actually done and there is nothing left, silence is the correct response -- do NOT write ceremony text to dodge this gate.',
  BOYSCOUT_CLEAN:
