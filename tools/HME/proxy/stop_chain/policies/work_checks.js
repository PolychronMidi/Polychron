...
const REASONS = {
  ...
  COMMENT_BLOAT:
    'COMMENT-BLOAT VIOLATION (doc/templates/doc/templates/AGENTS.md): A file you edited this turn contains a 5+ line consecutive inline-comment block. Rule: "Inline comments single-line and terse. Elaboration goes in doc/." Trim the block to <=2 lines OR move the explanation into doc/. Run `python3 scripts/audit-comment-bloat.py --files <path>` to see the offending blocks. If the work is actually done and there is nothing left, silence is the correct response -- do NOT write ceremony text to dodge this gate.',
  BOYSCOUT_LOC:
    'LOC LIMIT VIOLATION (doc/templates/doc/templates/AGENTS.md): A file you Edited/Wrote this turn exceeds 350 LOC and is not in config/loc-ignore.txt. Rule: "Files MUST be <=350 LOC unless listed in loc-ignore.txt. Organize at logical boundaries." Either split the file, OR add to loc-ignore.txt with rationale. If the work is actually done and there is nothing left, silence is the correct response -- do NOT write ceremony text to dodge this gate.',
  BOYSCOUT_CLEAN:
...
const ENFORCEMENT_REMINDER =
  'STOP. Re-read doc/templates/doc/templates/AGENTS.md and the user prompt. Did you do ALL the work asked? Every change must be implemented in code, including errors that surface along the way in other involved tools or code (in /src, /tools, or wherever the request is scoped), not just documented. If you skipped anything, go back and do it now.';
