...
module.exports = {
  ...
  onToolResult({ toolUse, toolResult, ctx }) {
    ...
    // READ-SIDE FIRST: did THIS tool_use's input reference identifiers a
    // PRIOR enricher footer injected? If yes, bump that enricher's `acted`.
    ...
    // WRITE-SIDE: detect which enrichers fired on THIS tool_result, store
    // their injected identifiers keyed by tool_use_id for the NEXT scan.
    if (!text) return;
    const hits = {};
    const identifiers = [];
    const firedNames = [];
