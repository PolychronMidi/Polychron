...
// by proxy/self_origin.js (single source, contract-tested as the canonical
// superset). This module previously hand-maintained a SELF_TAG_RE/OBSERVATION_RE
...
function _sessionstartProxyDownResolved(projectRoot) {
  ...
  try {
    ...
    return status === 'ok';
  // silent-ok: bad sessionstart probe means SessionStart is not proven.
  } catch (_e) {
    return false;
  }
}
...
function _staleRuntimeResolvedOrGrace(projectRoot) {
  ...
  // silent-ok: bad stale-runtime marker disables grace; alerts remain eligible.
  } catch (_e) {
    return false;
  }
}
...
function _isAgentActionable(line, projectRoot) {
  ...
  if (selfOrigin.isSelfOrigin(body) && !criticalInfra) return false;
...
function _rotateIfNeeded(errLogPath, wmPath, totalLines, lines) {
  ...
  try {
    ...
    // silent-ok: rotation failure leaves original log and uses unrotated lines.
...
module.exports = {
  ...
  onRequest({ payload, ctx }) {
    ...
    // silent-ok: watermark write failure emits event and aborts injection.
    } catch (err) {
