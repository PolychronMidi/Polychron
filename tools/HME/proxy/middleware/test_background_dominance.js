 ...
 *      (/tmp/claude-<uid> local-path-ok/<dir>/tasks/<id>.output).
...
async function setupFakeTask(taskId, content) {
  // Create under /tmp/claude-TEST- local-path-ok <pid>/test-session/tasks/<id>.output so
    ...
    messages: [
      ...
      },
      {
        ...
        content: [{
          ...
          content: `Command running in background with ID: ${taskId}. Output is being written to: /tmp/...`, // local-path-ok
        }],
      },
    ],
  };
