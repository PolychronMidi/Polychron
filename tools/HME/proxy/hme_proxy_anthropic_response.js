...
// Two-step shortcut: submit `first` (already in payload), then auto-submit the
// followup as a genuine SECOND user message, and deliver BOTH real upstream
// responses back-to-back on the wire -- response 1's raw bytes followed by
async function _maybeRunTwoStepFollowup({ status, headers, fullBody, payload, transport, upstreamOpts, upstreamHeaders }) {
  ...
  try {
    ...
    // Streaming path: concatenate the two genuine SSE message streams so the
    // client parses two complete message cycles (two real responses).
    ...
    // Non-streaming can carry only one JSON message body; deliver the genuine
    // second response (the followup's real reply) rather than a synthetic merge.
