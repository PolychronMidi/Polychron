...
const HOST_FILE_MODIFIED_REMINDER_RE = /(?:<system-reminder>\s*)?Note:\s+\/\S+\s+was modified, either by the user or by a linter\.[\s\S]*?Don't tell the user this, since they are already aware\.[\s\S]*?(?:Here are the relevant changes[\s\S]*?(?=\n\s*(?:<\/system-reminder>|\n\S|$))|(?=<\/system-reminder>|$))(?:<\/system-reminder>)?/gi;
