...
function applyOverdriveRoute({ payload, clientReq, clientRes, outBody, stripStaleToolResults, stripClaudeIdentity, shrinkForContext, env = process.env, projectRoot = PROJECT_ROOT, cfg = loadModelsJson() }) {
  ...
  if (prefixedClaudeModel) {
    result.wasStreaming = payload.stream === true;
    result.injected = true;
    let chainInfo;
    try { chainInfo = buildMode1Chain({ ...payload, model: prefixedClaudeModel }, env, cfg, { projectRoot }); }
    catch (err) { console.error(`[hme-proxy] MODE=1 fallback chain build failed for Claude-primary route: ${err.message}`); chainInfo = { chain: [], role: '', tier: modelTier(prefixedClaudeModel) }; }
    const primary = findAnthropicModelByApiId(cfg, prefixedClaudeModel) || { id: prefixedClaudeModel, api_model: prefixedClaudeModel, provider: 'anthropic' };
    ...
    result.swapModel = prefixedClaudeModel;
    result.omniProvider = 'anthropic';
    result.swapMeta = primary;
    payload.model = prefixedClaudeModel;
