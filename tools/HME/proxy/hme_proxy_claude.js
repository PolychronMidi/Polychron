...
function handleAnthropicResponseComplete(...args) {
  ...
  // is still initializing hme_proxy_claude. Leaf import (not the
...
const { acquireOpusSlotIfNeeded } = require('./hme_proxy_opus_gate');
const {
...
function createClaudeHandler(deps) {
  ...
  function handleRequest(clientReq, clientRes) {
    ...
    clientReq.on('end', async () => {
      ...
      // Final outbound context-budget gate (see outbound_context_gate.js).
