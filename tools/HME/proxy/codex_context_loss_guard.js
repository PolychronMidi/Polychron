...
const RECOVERED_RE = /\b(?:only\s+(?:have\s+)?(?:the\s+)?)?recovered\s+(?:tool\s+)?(?:result|context|state)\b/i;
const NO_CONTEXT_RE = /\b(?:no actual context apart from a failed call|no additional actionable project context|nothing useful came from it|failed invocation due to a missing command)\b/i;
const ASK_RESEND_RE = /\bplease\s+(?:send|provide)\s+(?:the\s+)?(?:actual\s+)?(?:task|command|goal|file|bug|repository goal|instructions?)\b/i;
const NO_ACTION_RE = /\b(?:no command was executed|no files were read or modified|there(?:'s| is) no additional actionable)\b/i;
