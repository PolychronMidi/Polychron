...
const NO_ACTION_RE = /\b(?:no command was executed|no files were read or modified|there(?:'s| is) no additional actionable|won[’']t repeat the empty Bash calls|avoid repeating (?:the )?(?:same )?(?:discovery\/listing|context check|tool)? ?calls|reuse the recovered repository context)\b/i;
