...
function tailFileLines(filepath, maxLines, maxBytes = 500_000) {
  try {
    ...
    // silent-ok: bad status tail adds no lines; other status still renders.
    return [];
  }
}
...
function recentLifesaverErrors() {
  ...
  const filtered = fresh.filter((line) => {
    ...
    if (selfOrigin.isAgentActionableOverride(line)) return true;
    if (selfOrigin.isSelfOriginSuppressed(line)) return false;
...
function coherenceStatusLine() {
  try {
    ...
    // silent-ok: bad coherence telemetry hides only optional status line.
    return null;
  }
}
