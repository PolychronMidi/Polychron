...
const TOOL_RENDER_ECHO_RE = /(?:^|\n)[ \t]*(?:Called|Calld)\s+(?:the\s+)?(?:Read|Edit|Write|Bash|Agent|WebFetch|WebSearch|MultiEdit)\s+tool\s+w(?:ith)?\s+(?:the\s+)?(?:following\s+)?input:[ \t]*\{[\s\S]{0,20000}?(?:File\s+\S[^\n]*?(?:has\s+been\s+updated\s+successfully|been\s+updatd\s+succssfly)[^\n]*|Succ?essfully\s+wrote[^\n]*|Succssfly\s+wrote[^\n]*|Result\s+of\s+(?:calling|calln)\s+(?:the\s+)?(?:Read|Edit|Write|Bash|Agent|WebFetch|WebSearch|MultiEdit)\s+tool[^\n]*(?:\n[ \t]*File\s+\S[^\n]*?(?:updated\s+successfully|updatd\s+succssfly)[^\n]*)?)/gi;
...
function stripHookUiEchoText(text, stats = {}, opts = {}) {
  ...
  out = out.replace(HOST_NATIVE_TOOL_ERROR_LINE_RE, (block) => (NATIVE_EDIT_ERROR_PHRASE_RE.test(block) ? removeBlock(block, true) : block));
