...
function shrinkForPassthrough(payload, opts = {}) {
  ...
  const stopMicrocompact = typeof opts.microcompactStop === 'function' ? opts.microcompactStop : null;
  let elided = 0;
  let microcompactStopped = false;
  for (let i = 0; i < recentStart && !microcompactStopped; i += 1) {
    const m = msgs[i];
    ...
    for (const b of m.content) {
      ...
      elided += 1;
      if (stopMicrocompact && stopMicrocompact({ payload, elided, threshold, maxTier })) {
        microcompactStopped = true;
        break;
      }
    }
  }
  if (elided > 0) {
    ...
    const targetReachedByTokens = forceCompaction && targetTokens > 0 && afterTokens > 0 && afterTokens <= targetTokens;
    if ((!forceCompaction || targetReachedByTokens) && afterBytes <= threshold) {
  ...
  let tailElided = 0;
  serialized = JSON.stringify(payload);
  if (allowMessageDrop && serialized.length > threshold) {
