...
def _load_trace_local() -> list[dict]:
    ...
    return _load_trace_impl(os.path.join(ctx.PROJECT_ROOT, "src", "output", "metrics", "trace.jsonl"))
...
def composition_critique() -> str:
    ...
    replay_path = os.path.join(ctx.PROJECT_ROOT, "src", "output", "metrics", "trace-replay.json")
