...
_LIFESAVER_MAX_OPEN = int(os.environ.get("HME_LIFESAVER_TODO_MAX_OPEN", "20"))  # env-ok: runtime/test tuning
_LIFESAVER_TTL_SECONDS = int(os.environ.get("HME_LIFESAVER_TODO_TTL_SEC", str(24 * 3600)))  # env-ok: runtime/test tuning
# Prune-after: how long a DONE lifesaver entry stays in the store before
_LIFESAVER_PRUNE_AFTER_SECONDS = int(os.environ.get("HME_LIFESAVER_TODO_PRUNE_SEC", str(3 * 24 * 3600)))  # env-ok: runtime/test tuning
# Prune-after for non-lifesaver done todos (native, hme_todo, todo_md).
_DONE_TODO_PRUNE_AFTER_SECONDS = int(os.environ.get("HME_DONE_TODO_PRUNE_SEC", str(7 * 24 * 3600)))  # env-ok: runtime/test tuning
