...
def merge_native_todowrite(incoming: list) -> list:
    ...
    updatedInput for the real TodoWrite call. Result is ordered:
      ...
      2. agent's incoming native items
      3. other persistent HME items the agent didn't include
    ...
    with _todo_lock:
        ...
        # Append HME-only items that the agent's native payload didn't include.
        for t in store:
