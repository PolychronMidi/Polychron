...
def suggest_evolution() -> str:
    ...
    trace_path = os.path.join(ctx.PROJECT_ROOT, "src", "output", "metrics", "trace-summary.json")
    ...
    perc_path = os.path.join(ctx.PROJECT_ROOT, "src", "output", "metrics", "perceptual-report.json")
    ...
    trace_records: list = []
    trace_raw_path = os.path.join(ctx.PROJECT_ROOT, "src", "output", "metrics", "trace.jsonl")
    ...
    # 4. Narrative excerpt
    narrative_path = os.path.join(ctx.PROJECT_ROOT, "src", "output", "metrics", "narrative-digest.md")
    ...
    model_path = os.path.join(ctx.PROJECT_ROOT, "src", "output", "metrics", "verdict-model.json")
