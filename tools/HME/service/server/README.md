...
`tools_analysis/` holds the agent-facing tool handlers (evolve, review, learn, trace, internal hme_admin, etc.); `tools_knowledge.py`, `tools_search.py`, `tools_index.py` hold internal low-level helpers those public tools dispatch to. `onboarding_chain.py` gates tool calls during the 7-step onboarding state machine.
