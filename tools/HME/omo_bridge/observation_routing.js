...
async function routeObservationEvent(universalEvent, options = {}) {
  ...
  if (!options.pluginHost || options.enabled === false) {
    emitSafe(emit, 'universal_hook_observation_skipped', { phase: event.phase, host: event.source.host });
  ...
  try {
    ...
    emitSafe(emit, 'universal_hook_observation_routed', { phase: event.phase, host: event.source.host, effects: safeEffects.length });
    return { routed: true, liveDecisionChanged: false, primaryDecision: { kind: 'allow' }, effects: safeEffects, pluginResult: result };
  } catch (error) {
    emitSafe(emit, 'universal_hook_observation_error', { phase: event.phase, host: event.source.host, error: error.message });
