...
import { execFileWithPassword, getErrorMessage, quotePowerShell, runElevatedPowerShell, } from "../systemCommands.js";
...
export async function addDNSEntry(sudoPassword) {
    ...
    try {
        if (IS_WIN) {
            await runElevatedPowerShell(`Add-Content -LiteralPath ${quotePowerShell(HOSTS_FILE)} -Value ${quotePowerShell(entry)}`);
        }
        else {
            await execFileWithPassword("sudo", ["-S", "tee", "-a", HOSTS_FILE], sudoPassword, `${entry}\n`);
        }
...
export async function removeDNSEntry(sudoPassword) {
    ...
    try {
      ...
      `);
            ...
            await execFileWithPassword("sudo", ["-S", process.execPath, "-e", REMOVE_HOSTS_ENTRY_SCRIPT, HOSTS_FILE, TARGET_HOST], sudoPassword);
        }
