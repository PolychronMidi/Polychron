        ...
        if (validatedProgression.length === 0) {
            ...
            switch (direction.toUpperCase()) {
                ...
                case '?':
                    ...
                    break;
                default:
                    console.warn('Invalid direction,defaulting to right');
                    next = 1;
            }
            let startingMeasure = measureCount;
            let progressChord = measureCount > startingMeasure || rf() < .05;
            if (progressChord && typeof subdivStart !== 'undefined') {
                allNotesOff(subdivStart);
                startingMeasure = measureCount;
            }
...
class TensionReleaseComposer extends ChordComposer {
    ...
    calculateTension(chordSymbol) {
        ...
        if ([0, 5].includes(scaleIndex))
            return 0.2;
        // Subdominant (ii, IV) = medium tension
        if ([1, 3].includes(scaleIndex))
            return 0.5;
        // Dominant (V, vii) = high tension
        if ([4, 6].includes(scaleIndex))
            return 0.9;
