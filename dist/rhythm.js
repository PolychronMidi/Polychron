...
getRhythm = (level, length, pattern, method, ...args) => {
    ...
    if (method) {
        ...
        const filteredRhythms = Object.fromEntries(Object.entries(rhythms).filter(([_, { weights }]) => weights[levelIndex] > 0));
 ...
 * @returns {void}
 */
trackBeatRhythm = () => { if (beatRhythm[beatIndex] > 0) {
    beatsOn++;
    beatsOff = 0;
}
else {
 ...
 * @returns {void}
 */
trackDivRhythm = () => { if (divRhythm[divIndex] > 0) {
    divsOn++;
    divsOff = 0;
}
else {
...
trackSubdivRhythm = () => { if (subdivRhythm[subdivIndex] > 0) {
    subdivsOn++;
    subdivsOff = 0;
}
else {
...
trackSubsubdivRhythm = () => { if (subsubdivRhythm[subsubdivIndex] > 0) {
    subsubdivsOn++;
    subsubdivsOff = 0;
}
else {
