...
* `Time`: Absolute time at which this event occurs. By default a number of MIDI clocks (ticks, rounded to nearest whole number). As a convenience, a time value prefixed with `t` (for example `t1.5`) denotes absolute time in seconds; the converter resolves `t` values by locating the most recent tempo or meter change at or before that time and applying that change's ticks‑per‑second rate to the delta seconds since the change (this anchors `t` conversions to the current measure/time segment). When a non‑power‑of‑two time signature denominator is encountered, the parser silently normalizes the denominator to the nearest MIDI‑friendly power of two and inserts an implicit tempo adjustment (sync‑factor) so playback aligns musically. Meta-events for which time is not meaningful (for example, song title, copyright information, etc.) have an absolute time of 0.
