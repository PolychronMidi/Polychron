...
- `check-manifest-health.js` — validates `metrics/feedback_graph.json`, registration manifests, and port declarations
...
- `check-hme-coherence.js` — reads `metrics/hme-activity.jsonl`; fails on `coherence_violation` events in the current round
...
3. Errors must go to `metrics/pipeline-summary.json` under `errorPatterns` (not just stdout — non-fatal steps get scanned)
...
rules:
  ...
  - Non-fatal validator errors must be written to `metrics/pipeline-summary.json` errorPatterns — stdout alone gets missed by the LIFESAVER scanner
