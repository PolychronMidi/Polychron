...
- **New feedback loops** must register with `feedbackRegistry` to prevent catastrophic resonance. Declare in `doc/FEEDBACK_GRAPH.json` and ensure `scripts/validate-feedback-graph.js` passes.
...
- [doc/FEEDBACK_GRAPH.json](../doc/FEEDBACK_GRAPH.json) - Feedback loop topology (source of truth for visualization). 8 loops, auto-generated by `scripts/generate-feedback-graph.js` and cross-validated by `scripts/validate-feedback-graph.js` on every pipeline run.
