...
**HME is your nervous system.** Load it first (`/HME`). Every HME tool is invoked as a Bash call against a shell wrapper in `i/` — see the cheatsheet in the frontmatter comment. Use `i/hme-read target=<module> mode=before` before every file change. Use `i/trace target=<query>` for callers/boundary/semantic navigation instead of Grep. Use `i/review mode=forget` after changes. Use `i/learn title="…" content="…"` after confirmed rounds. Use `i/trace target="error text" mode=diagnose` on pipeline failures. See [doc/HME.md](../../doc/HME.md) for the full tool reference.
...
Start with `i/review mode=changes` to see what changed since last round with KB context. Then read metrics in order. Tier 1 gives the headline. Tier 2 gives the full picture. Tier 3 is reference.
...
This is the phase that matters. Use `i/learn query=<topic>` to check what the KB already knows about areas you're investigating. Use `i/trace target="callers of X"` and `i/trace target=<module>` to trace causal chains through the codebase.
...
- **Before modifying any file:** `i/hme-read target=<moduleName> mode=before` for KB constraints + callers + boundaries
- **For open-ended searches:** `i/trace target=<query>` — auto-routes callers/boundary/grep/semantic. NOT Grep
- **After changes:** `i/review mode=forget` — auto-detects changed files from git
- **After confirmed round:** `i/learn title="…" content="…" category=pattern` for calibration anchors
- **When pipeline fails:** `i/trace target="error text" mode=diagnose` for source trace + similar bugs
