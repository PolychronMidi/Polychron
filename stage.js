...
stutterFade=(channels,numStutters=ri(10,70),duration=ticksPerSecond*rf(.2,1.5))=>{
  ...
  channelsArray.forEach(channelToStutter => { const maxVol=ri(90,120);
    ...
    for (let i=m.floor(numStutters*(rf(1/3,2/3))); i < numStutters; i++) {
      currentTick=beatStart + i * (duration/numStutters) * rf(.9,1.1);
      if (isFadeIn) {
      ...
      p(c, {tick:currentTick + duration*rf(.95,1.95), type:'control_c', vals:[channelToStutter, 7, volume]});
    }
    p(c, {tick:currentTick + duration*rf(.5,3), type:'control_c', vals:[channelToStutter, 7, maxVol]});
  });
};

stutterPan = (channels, numStutters = ri(30,90), duration = ticksPerSecond*rf(.1,1.2)) => {
  ...
  channelsArray.forEach(channelToStutter => { 
    ...
    for (let i = m.floor(numStutters*(rf(1/3))); i < numStutters; i++) {
      currentTick = beatStart + i * (duration/numStutters) * rf(.7,1.3);
      if (isFadeIn) {
      ...
      p(c, {tick:currentTick + duration*rf(.5,1.75), type:'control_c', vals:[channelToStutter, 10, pan]});
    }
    p(c, {tick:currentTick + duration*rf(), type:'control_c', vals:[channelToStutter, 10, ri(58,70)]});
  });
};

stutterFX = (channels, numStutters = ri(30,100), duration = ticksPerSecond*rf(.1,2)) => {
  const CHsToStutter = ri(1,2); const channelsToStutter = new Set();
  ...
  channelsArray.forEach(channelToStutter => { 
    ...
    for (let i = m.floor(numStutters*(rf(1/3))); i < numStutters; i++) {
      ...
      p(c, {tick:currentTick + duration*rf(.75,1.5), type:'control_c', vals:[channelToStutter, FXToStutter, pan]});
    }
    p(c, {tick:currentTick + duration*rf(), type:'control_c', vals:[channelToStutter, FXToStutter, ri(58,70)]});
  });
};
...
setNoteParams=()=>{
  ...
  binauralVelocity=rv(velocity * rf(.42,.57));
}
...
composer.getNotes().forEach(({ note })=>{ source.filter(sourceCH=>
  ...
  p(c,{tick:sourceCH===centerCH1 ? on + rv(ticksPerSubdiv*rf(1/9),[-.1,.1],.3) : on + rv(ticksPerSubdiv*rf(1/3),[-.1,.1],.3),type:'on',vals:[sourceCH,note,sourceCH===centerCH1 ? velocity*rf(.95,1.15) : binauralVelocity*rf(.95,1.03)]});
  ...
  if (globalStutterData) {
    ...
    for (let i=0; i < numStutters; i++) {
      const currentTick=on + duration * i; let stutterNote=note;
      if (rf() < .25) {
      ...
      p(c,{tick:currentTick - duration * rf(.15),vals:[sourceCH,stutterNote]});
      p(c,{tick:currentTick + duration * rf(.15,.6),type:'on',vals:[sourceCH,stutterNote,sourceCH===centerCH1 ? currentVelocity * rf(.3,.7) : currentVelocity * rf(.45,.8)]});
    }
  ...
  if (rf()<rv(.1,[.5,1],.3)){ // Source Channels Stutter-Shift #2: Unique per channel.
    ...
    for (let i=0;i<numStutters;i++) {
      ...
      p(c,{tick:currentTick-duration*rf(.15),vals:[sourceCH,stutterNote]});
      p(c,{tick:currentTick+duration*rf(.15,.6),type:'on',vals:[sourceCH,stutterNote,sourceCH===centerCH1?velocity*rf(.3,.7):binauralVelocity*rf(.45,.8)]});
    }
    p(c,{tick:on+sustain*rf(.5,1.5),vals:[sourceCH,note]});
  }

  reflectionCH=reflect[sourceCH]; 
  p(c,{tick:reflectionCH===centerCH2 ? on+rv(ticksPerSubdiv*rf(.2),[-.01,.1],.5) : on+rv(ticksPerSubdiv*rf(1/3),[-.01,.1],.5),type:'on',vals:[reflectionCH,note,reflectionCH===centerCH2 ? velocity*rf(.5,.8) : binauralVelocity*rf(.55,.9)]});
  ...
  if (rf()<.33){ // Reflection Channels Stutter-Shift
    ...
    for (let i=0;i<numStutters;i++) {
      ...
      p(c,{tick:currentTick-duration*rf(.3),vals:[reflectionCH,stutterNote]});
      p(c,{tick:currentTick+duration*rf(.25,.7),type:'on',vals:[reflectionCH,stutterNote,reflectionCH===centerCH2?velocity*rf(.25,.65):binauralVelocity*rf(.4,.75)]});
    }
  ...
  if (rf()<clamp(.45*bpmRatio3,.2,.7)) {
    ...
    p(c,{tick:bassCH===centerCH3 ? on+rv(ticksPerSubdiv*rf(.1),[-.01,.1],.5) : on+rv(ticksPerSubdiv*rf(1/3),[-.01,.1],.5),type:'on',vals:[bassCH,bassNote,bassCH===centerCH3 ? velocity*rf(1.15,1.35) : binauralVelocity*rf(1.85,2.45)]});
    ...
    if (rf()<.7){ // Bass Channels Stutter-Shift
      ...
      const duration=.25 * ri(1,8) * sustain / numStutters;
      for (let i=0;i<numStutters;i++) {
        const currentTick=on+duration*i; let stutterNote=bassNote;
        if(rf()<.5){
        ...
        p(c,{tick:currentTick-duration*rf(.3),vals:[bassCH,stutterNote]});
        p(c,{tick:currentTick+duration*rf(.25,.7),type:'on',vals:[bassCH,stutterNote,bassCH===centerCH3?velocity*rf(.55,.85):binauralVelocity*rf(.75,1.05)]});
      }
...
composer.getNotes().forEach(({ note })=>{ source.filter(sourceCH=>
  ...
  p(c,{tick:sourceCH===centerCH1 ? on + rv(ticksPerSubsubdiv*rf(1/9),[-.1,.1],.3) : on + rv(ticksPerSubsubdiv*rf(1/3),[-.1,.1],.3),type:'on',vals:[sourceCH,note,sourceCH===centerCH1 ? velocity*rf(.95,1.15) : binauralVelocity*rf(.95,1.03)]});
  p(c,{tick:on+sustain*(sourceCH===centerCH1 ? 1 : rv(rf(.92,1.03))),vals:[sourceCH,note]});
  ...
  if (!stutterApplied && rf() < rv(.2,[.5,1],.3)) {
    // Calculate stutter once for all Source channels
    globalStutterData={
      numStutters: m.round(rv(rv(ri(3,9),[2,5],.33),[2,5],.1)),
      duration: .25 * ri(1,6) * sustain / numStutters,
      minVelocity: 11,
      maxVelocity: 111,
      isFadeIn: rf() < 0.5,
  ...
  reflectionCH=reflect[sourceCH]; 
  p(c,{tick:reflectionCH===centerCH2 ? on+rv(ticksPerSubsubdiv*rf(.2),[-.01,.1],.5) : on+rv(ticksPerSubsubdiv*rf(1/3),[-.01,.1],.5),type:'on',vals:[reflectionCH,note,reflectionCH===centerCH2 ? velocity*rf(.5,.8) : binauralVelocity*rf(.55,.9)]});
  p(c,{tick:on+sustain*(reflectionCH===centerCH2 ? rf(.7,1.2) : rv(rf(.65,1.3))),vals:[reflectionCH,note]});
  ...
  if (rf()<clamp(.35*bpmRatio3,.2,.7)) {
    bassCH=reflect2[sourceCH]; bassNote=modClamp(note,12,35);
    p(c,{tick:bassCH===centerCH3 ? on+rv(ticksPerSubsubdiv*rf(.1),[-.01,.1],.5) : on+rv(ticksPerSubsubdiv*rf(1/3),[-.01,.1],.5),type:'on',vals:[bassCH,bassNote,bassCH===centerCH3 ? velocity*rf(1.15,1.35) : binauralVelocity*rf(1.85,2.45)]});
    p(c,{tick:on+sustain*(bassCH===centerCH3 ? rf(1.1,3) : rv(rf(.8,3.5))),vals:[bassCH,bassNote]});
    if (rf()<.7){ // Bass Channels Stutter-Shift
      if (!stutters.has(bassCH)) stutters.set(bassCH,m.round(rv(rv(ri(2,5),[2,3],.33),[2,10],.1)));
      const numStutters=stutters.get(bassCH);
