moduleLifecycle.declare({
  ...
  function axisEnergyEquilibratorAxisAdjustmentsApplyAxisLoop(state, config, context, V) {
    ...
    for (let i = 0; i < config.ALL_AXES.length; i++) {
      ...
      if (share > config.AXIS_OVERSHOOT && axisTightenScale > 0) {
        ...
        // Flicker overshoot needs stronger dampening; #4 manages range, not share.
        ...
        for (let p = 0; p < pairs.length; p++) {
          ...
          // Cross-inhibit recent pair relaxation.
        ...
        // Override phaseHot at critically low share; p95/baseline can inflate heat.
        ...
        const nonNudgeableRelaxBoost = context.nonNudgeableTailPressure > 0 && context.nonNudgeableAxes.indexOf(axis) !== -1
          ...
          : 1.0;
        // Phase boost multipliers come from the adaptive phase floor controller.
        ...
        // Symmetric giniMult lets starved axes recover faster in imbalanced states.
        ...
        for (let p = 0; p < pairs.length; p++) {
          ...
          // Cross-inhibit recent pair tightening.
  ...
  function axisEnergyEquilibratorAxisAdjustmentsApplySpecialCaps(state, config, V) {
    ...
    // Removed entropy-cap-0.19; generic AXIS_OVERSHOOT covers rare spikes.

    // Removed manual entropy floor; generic undershoot recovery owns it.

    // Removed phase-trust seesaw; phase/trust recovery have dedicated controllers.

    const trustSmoothed = state.smoothedShares.trust;

    // Trust floor gently relaxes trust-pair baselines below 0.14 share.
    if (typeof trustSmoothed === 'number' && trustSmoothed < 0.14 && trustSmoothed > 0.001) {
      ...
      for (let i = 0; i < trustFloorPairs.length; i++) {
        ...
        // Skip non-nudgeable pairs so trust recovery budget hits active pairs.
