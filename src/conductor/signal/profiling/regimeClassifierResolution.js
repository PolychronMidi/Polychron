moduleLifecycle.declare({
  ...
  subsystem: 'conductor',
  deps: ['L0', 'validator'],
  lazyDeps: ['explainabilityBus', 'phaseFloorController', 'pipelineCouplingManager', 'regimeClassifierHelpers'],
  ...
  function activateForcedRegime(state, config, regime, reason, beatsRemaining, triggerStreak, triggerTickId) {
    ...
    L0.post(L0_CHANNELS.regimeTransition, 'both', beatStartTime, { from: state.lastRegime, to: regime, cause: 'forced-' + reason });
  }
    ...
    if (state.forcedRegimeBeatsRemaining > 0) {
      ...
      if (windowHits >= requiredHits) {
        ...
        if (allowTransition) {
          ...
          L0.post(L0_CHANNELS.regimeTransition, 'both', beatStartTime, { from: state.lastRegime, to: rawRegime, cause: 'organic-' + organicCause });
