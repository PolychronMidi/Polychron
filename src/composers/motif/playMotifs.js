...
// Handles division-level (divMotifs) bucket retrieval, cycle tracking, transformations, and voice coordination
//
...
playMotifs = /** @type {any} */ (function playMotifs(unit = 'subdiv', layer) {
  // Validate layer and division-level motif buckets
  if (!layer || !layer.divMotifs) {
    throw new Error(`${unit}.playMotifs missing layer or divMotifs`);
  }

  // Compute absolute division index for the current call (DIVS only).
  // Use the planner's `divsPerBeat` (stored on the layer at planning time) so
  // runtime indexing matches the planner even if global `divsPerBeat` changed.
  ...
  const absDivBase = Number.isFinite(Number(beatIndex)) ? Number(beatIndex) : 0;
  const absDivOffset = Number.isFinite(Number(divIndex)) ? Number(divIndex) : 0;
  const absDivIndex = m.max(0, absDivBase * Number(plannedDivsPerBeat) + absDivOffset);

  // Initialize per-division cursor so we can cycle through bucket notes on each call
  if (typeof layer._lastDivIndex !== 'number' || absDivIndex < layer._lastDivIndex) {
    // New measure or reset; clear cursors
    layer._divBucketCursor = new Map();
  }
  layer._lastDivIndex = absDivIndex;
  if (!layer._divBucketCursor) layer._divBucketCursor = new Map();

  // For `beat` unit map to the first division of the beat (use planner's divs)
  const targetDivIndex = (unit === 'beat') ? (absDivBase * Number(plannedDivsPerBeat)) : absDivIndex;
  if (!Number.isFinite(Number(targetDivIndex)) || targetDivIndex < 0) {
    throw new Error(`${unit}.playMotifs: invalid targetDivIndex computed`);
  }

  // Fail-fast: divMotifs must be populated by the planner. Surface errors loudly
  // so upstream planning/timeing issues are fixed instead of silently masked.
  if (!layer || !layer.divMotifs || !Array.isArray(layer.divMotifs[targetDivIndex])) {
    throw new Error(`${unit}.playMotifs missing divMotifs bucket for divIndex ${targetDivIndex} - fail-fast: planning did not populate division buckets`);
  }

  const bucket = layer.divMotifs[targetDivIndex];

  // If we have an explicit bucket but it's empty, fail loudly
  if (!bucket || bucket.length === 0) {
    throw new Error(`${unit}.playMotifs empty divMotifs bucket at divIndex ${targetDivIndex} - fail-fast: generation failed to populate`);
  }
  ...
  const cursor = layer._divBucketCursor.get(targetDivIndex) ?? 0;
  const bucketEntry = bucket[cursor % bucket.length];
  layer._divBucketCursor.set(targetDivIndex, cursor + 1);
...
/** @type {any} */ (playMotifs).resetLayerState = function(layer) {
  ...
  // Clear division buckets to avoid stale notes across phrases
  layer.divMotifs = [];
