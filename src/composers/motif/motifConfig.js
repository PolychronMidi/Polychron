...
motifConfig = (function() {
  ...
  const UNIT_PROFILES = (typeof MOTIF_UNIT_PROFILES !== 'undefined' && MOTIF_UNIT_PROFILES && typeof MOTIF_UNIT_PROFILES === 'object')
    ? MOTIF_UNIT_PROFILES
    : LOCAL_UNIT_PROFILES;
  ...
  function getProfile(name) {
    ...
    const source = (typeof MOTIF_PROFILES !== 'undefined' && MOTIF_PROFILES) ? MOTIF_PROFILES : (console.warn('Acceptable warning: motifConfig: using local defaults. For project-wide settings, define MOTIF_PROFILES in src/conductor/config.js.'), LOCAL);
    const p = source[name];
