...
voiceLeadingCore = {
  ...
  computeCandidateScore(scorer, candidate, lastNotes, registerRange, constraints, opts = {}) {
    ...
    const baseCtWeight = V.optionalFinite(opts && opts.commonToneWeight, scorer.commonToneWeight);
    const ctWeightMod = applyVoiceLeadingWeightNoise(1.0, 'commonTone', noiseContext);
    const ctWeight = Number.isFinite(baseCtWeight) ? (baseCtWeight * ctWeightMod) : 0;
    if (ctWeight > 0) {
