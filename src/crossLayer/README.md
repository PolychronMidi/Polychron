...
The write-firewall to conductor is strict and enforced by lint (`local/no-direct-crosslayer-write-from-conductor` and its inverse) and by the 9 declared firewall ports in `src/output/metrics/feedback_graph.json`. Any new cross-boundary data flow must declare a port.
...
See [src/output/metrics/feedback_graph.json](../../src/output/metrics/feedback_graph.json) for the port topology and [doc/composition-full.md](../../doc/composition-full.md) for the boundary rationale.
