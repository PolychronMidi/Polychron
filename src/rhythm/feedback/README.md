...
`emissionFeedbackListener` and `conductorRegulationListener` are closed feedback loops. Both must be declared in `metrics/feedback_graph.json`. New listeners added here follow the same contract: register via `feedbackRegistry`, declare the port.

<!-- HME-DIR-INTENT
rules:
  - feedbackAccumulator loads first — all listeners depend on it; never reorder it below any listener in index.js
  - New listeners must register via feedbackRegistry and declare a port in metrics/feedback_graph.json
-->
