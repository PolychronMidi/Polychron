...
if (require.main === module) {
  ...
  (function _refuseIfLocked() {
    ...
    let _pid = 0;
    try { _pid = Number(_fs.readFileSync(_lockPath, 'utf8').trim()); } catch (_e) { /* malformed lock file -- treat as stale below; silent-ok: malformed lock file is a stale-lock path */ }
    if (_pid > 0 && _pid !== process.pid) {
      try {
        ...
        process.exit(2);
      } catch (_e) { /* PID dead -- stale lock, run-with-log will clear it; silent-ok: dead PID is a valid stale-lock signal */ }
    }
  })();
  main().catch((err) => {
