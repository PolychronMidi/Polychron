...
Sketch playground for prototyping behavior before promoting to `src/`. Each sketch in `sketches.js` monkey-patches globals or functions at load time via `postBoot()` and runs the engine against an isolated temp working directory — never touches the real `output/`.
...
- `run.js` — sketch runner; creates isolated temp cwd, runs the engine, emits `.wav` to `lab/output/`
...
```bash
node lab/run.js                          # all sketches
node lab/run.js <sketch-name> ...        # specific sketches
```
...
rules:
  ...
  - Lab output is isolated in temp cwd; never write to `rootDir/output/` from a sketch
-->
