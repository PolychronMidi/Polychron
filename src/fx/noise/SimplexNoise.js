...
SimplexNoise = class {
  ...
  noise(xin, yin) {
    const s = (xin + yin) * F2;
    const i = m.floor(xin + s);
    const j = m.floor(yin + s);
    const t = (i + j) * G2;
    const X0 = i - t;
    const Y0 = j - t;
    const x0 = xin - X0;
    ...
    const n0 = (t0 < 0) ? 0 : (t0 * t0) ** 2 * this.dot(grad3[gi0], x0, y0);
    const t1 = 0.5 - x1 * x1 - y1 * y1;
    const n1 = (t1 < 0) ? 0 : (t1 * t1) ** 2 * this.dot(grad3[gi1], x1, y1);
    const t2 = 0.5 - x2 * x2 - y2 * y2;
    const n2 = (t2 < 0) ? 0 : (t2 * t2) ** 2 * this.dot(grad3[gi2], x2, y2);
    return 70 * (n0 + n1 + n2);
  }
};
