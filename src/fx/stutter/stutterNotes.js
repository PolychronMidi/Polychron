...
stutterNotes = (/** @type {any} */ opts = {}) => {
  ...
  // Emit and metrics
  p(c, evOn);
  try { if (typeof EventBus !== 'undefined' && EventBus && typeof EventBus.emit === 'function') EventBus.emit('stutter-applied', { type: 'note', profile, channel, shift, intensity: clamp(stutterVel / 127, 0, 1), tick: stutterOn }); } catch { /* ignore */ }
  p(c, evOff);
