...
- `check-hme-coherence.js` -- reads `src/output/metrics/hme-activity.jsonl`; fails on `coherence_violation` events in the current round. As of Apr 2026 the `write_without_hme_read` emitter was retired (legacy-MCP contract replaced by auto-enrichment middleware); the check runs as a no-op unless future `coherence_violation` variants are introduced.
