...
- `check-manifest-health.js` -- validates `src/output/metrics/feedback_graph.json`, registration manifests, and port declarations
...
- `check-hme-coherence.js` -- reads `tools/HME/runtime/metrics/hme-activity.jsonl`; fails on `coherence_violation` events in the current round. As of Apr 2026 the `write_without_hme_read` emitter was retired (legacy-MCP contract replaced by auto-enrichment middleware); the check runs as a no-op unless future `coherence_violation` variants are introduced.
...
3. Errors must go to `src/output/metrics/pipeline-summary.json` under `errorPatterns` (not just stdout -- non-fatal steps get scanned)
...
rules:
  ...
  - Non-fatal validator errors must be written to `src/output/metrics/pipeline-summary.json` errorPatterns -- stdout alone gets missed by the LIFESAVER scanner
