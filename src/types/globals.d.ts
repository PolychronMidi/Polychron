...
declare var warmupRampController: { recordS0Exceedance(pair: string): void; getWarmupBeats(pair: string): number; getSnapshot(): any; reset(): void };
...
declare var hyperMetaOrchestrator: { getRateMultiplier(key: string): number; getPhaseBoostCeiling(): number; getP95AlphaMultiplier(): number; getS0TighteningMultiplier(): number; getSystemPhase(): 'converging' | 'oscillating' | 'stabilized'; recordExceedance(pair: string): void; getAxisConcentration(): { axisExceedance: Record<string, number>; concentration: number; dominantAxis: string }; getSnapshot(): any; reset(): void };
