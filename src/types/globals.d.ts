...
declare var hyperMetaManagerState: { ORCHESTRATE_INTERVAL: number; HEALTH_EMA_ALPHA: number; EFFECTIVENESS_EMA_ALPHA: number; INTERVENTION_BUDGET: number; TRUST_VELOCITY_DAMPING: number; PHASE_STALE_THRESHOLD: number; MAX_CONTRADICTIONS: number; S: any; controllerStats: Record<string, any>; rateMultipliers: Record<string, number>; contradictions: any[]; axisExceedanceCounts: Record<string, number>; pairExceedanceCounts: Record<string, number>; prevCorrSign: Record<string, number>; trajectory: any[]; prevFingerprint: Record<string, number>; reconciliationGaps: Record<string, any>; trustVelocityHistory: Record<string, number[]> };
