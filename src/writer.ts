...
export const grandFinale = (ctxOrEnv?: ICompositionContext | Record<string, any>): void => {
  ...
  layerData.forEach(({ name, buffer, state }, layerIndex) => {
    ...
    try {
      ...
      try { if (_shouldTrace_writer) console.error(`[grandFinale][DEBUG] layer=${name} sectionMaxEnd=${JSON.stringify(sectionMaxEnd)} sectionOffsets=${JSON.stringify(sectionOffsets)} sectionIndices=${JSON.stringify(sectionIndices)}`); } catch (_e) {}

      // Apply offsets to discovered units so units manifest records absolute ticks
      for (const u of unitsForLayer) {
        try { if (_shouldTrace_writer) console.error(`[grandFinale][DEBUG] beforeOffset unitHash=${u.unitHash} sectionIndex=${u.sectionIndex} start=${u.startTick} end=${u.endTick}`); } catch (_e) {}
        ...
        try { if (_shouldTrace_writer) console.error(`[grandFinale][DEBUG] afterOffset unitHash=${u.unitHash} sectionIndex=${u.sectionIndex} start=${u.startTick} end=${u.endTick}`); } catch (_e) {}
      }
    ...
    try {
      try { if (_shouldTrace_writer) console.error(`[grandFinale][DEBUG] layer=${name} timingTreePresent=${Boolean(env && env.state && env.state.timingTree && env.state.timingTree[name])} layerKeys=${env && env.state && env.state.timingTree && env.state.timingTree[name] ? Object.keys(env.state.timingTree[name]) : 'none'}`); } catch (_e) {}
      if ((!unitsForLayer || unitsForLayer.length === 0) && env && env.state && env.state.timingTree && env.state.timingTree[name]) {
          ...
          if (extracted && extracted.length > 0) {
            ...
            try { if (_shouldTrace_writer) console.error(`[grandFinale][DEBUG] layer=${name} extractedUnits=${extracted.length} sample=${JSON.stringify(extracted.slice(0,10))}`); } catch (_e) {}
  ...
  try {
    try { if (_shouldTrace_writer) console.error(`[grandFinale][DEBUG] unitsToWrite=${allUnits.length} sample=${JSON.stringify(allUnits.slice(0,10))}`); } catch (_e) {}
    const unitsManifest = {
