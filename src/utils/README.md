...
`feedbackRegistry` requires every feedback loop to be declared before use. New feedback loops: register at module load time, then declare in `src/output/metrics/feedback_graph.json`.

<!-- HME-DIR-INTENT
rules:
  - Always use trustSystems.names.* / trustSystems.heatMapSystems.* for trust system names -- never hardcode strings
  - New feedback loops must register via feedbackRegistry at load time AND be declared in src/output/metrics/feedback_graph.json
-->
