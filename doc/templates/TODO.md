...
- [easy] (w) `test_evasion_intent.py` + `test_scope_vs_shipped.py` wrapped in `_run_all()` + `if __name__ == "__main__":` guards. Previously they ran assertions and called `sys.exit()` at module level, causing pytest's collector to INTERNALERROR on import (seen in `log/hme-errors.log` at 09:48:31Z). Now pytest imports them silently with no `def test_*` symbols and walks away clean; standalone `python3 test_X.py` invocation still works. Both still pass 12/12 + 16/16. Landed 2026-05-10. (auto-shipped from SPEC checkbox flip)
