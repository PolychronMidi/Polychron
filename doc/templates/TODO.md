...
#27 3_ AUDIT-UNCERTAIN (mesh-found, 3-peer): team_agent_router.py main() allows native Agent dispatch when a team registry exists but no target matches (registry_exists branch -> permissionDecision allow + additionalContext), bypassing guard/ask-peer/channels/role-priming. Peers recommend deny-with-typed-reason; BUT denying native Agent in OVERDRIVE with a partial registry risks failing CLOSED on legitimate non-mesh Agent work. High-risk live permission-gate change -- needs explicit decision (which exact no-target states deny vs allow, override path) before implementation, like F1 enforcement.
#28 0_ P2 follow-up (mesh-found, downgraded): review_brief.py _CAP=300000 and team_dispatch_guard.py --context-cap default 24000 can disagree; round_measured.sh now passes --context-cap 300000 so the active runner is consistent, but a future guard caller without an explicit cap could lint-pass a brief the guard then truncates. Define one shared context-cap constant/env default for both the linter and the guard default.
