...
#15 0_ HME design-pattern cleanup: strengthen the host-neutral Decision algebra/reducer so policy code returns canonical allow/deny/instruct/rewrite/error decisions, not host hook JSON

#16 0_ HME design-pattern cleanup: add one event-kernel policy-result renderer that translates reduced decisions into hookSpecificOutput at the adapter boundary, including actual host event names

#17 0_ HME design-pattern cleanup: refactor dispatcher unified-policy handling to use the canonical reducer/renderer instead of building ad-hoc hookSpecificOutput branches

#18 0_ HME design-pattern cleanup: add regression tests pinning first-deny/rewrite/instruct rendering and forbidding dispatcher-side schema drift like missing hookEventName
