...
#3 1_ rewriters: structured-JSON bypass shard exists (`structured_output_guard.js`) and guards `sse_slop_rewriter` + `sse_ascii_strip_rewriter`; finish by wiring shared guarded text-block buffer for stop-hook text rewriters so every response-text rewriter skips valid structured JSON before mutation [E2]
