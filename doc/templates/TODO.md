...
#15 5_ Hook UI silence contract: automatic rewrite notes are telemetry-only across dispatcher and pre-write paths; `additionalContext` now surfaces only explicit `ctx.instruct(...)` messages while updatedInput still carries mechanical rewrites

#16 5_ Schema-boundary hardening: policy/dispatcher/pre-write paths route hook JSON through `event_kernel/decision_renderer.js`; regression test forbids direct `hookSpecificOutput` construction in dispatcher, pre-write, and builtin policies

#17 5_ Quarantine convergence proof: `proxy_slot_lifecycle.quarantineStatus()` and health output now distinguish blocked-current, fixed-build-waiting-for-supervisors, and cleared-by-viable-slot states without manual restarts
