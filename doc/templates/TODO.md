...
#15 0_ Hook UI silence contract: keep automatic rewrite/repair notes telemetry-only and reserve model-visible hook context for actionable denies/instructions

#16 0_ Schema-boundary hardening: forbid policy/dispatcher/pre-write code from constructing hookSpecificOutput directly outside the canonical renderer/adapters

#17 0_ Quarantine convergence proof: make proxy runtime quarantine state report current-vs-broken fingerprint and supervisor convergence status without manual restarts
