...
- [ ] [medium] (c.4-deep) Wire `tier_classifier.classify_heuristic()` into `userpromptsubmit.sh` so live turns produce fresh telemetry. Without this, the age-gate is the only defense against false positives; per-turn classification stops working entirely. Spawn-or-fold to a follow-on phase.
