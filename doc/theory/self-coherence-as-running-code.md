...
> `doc/templates/AGENTS.md` says what Polychron *can't be* (prohibitions). `tools/HME/runtime/metrics/hme-constitution.json` says what Polychron *fundamentally IS* (positive affirmations).
...
Generative versus prohibitive identity was a theoretical distinction in developmental psychology. HME has two separate files, `doc/templates/AGENTS.md` and `hme-constitution.json`, implementing the distinction at the level of system governance.
