...
- 🪐 **V** — Conjugate channel + coupling verifier + multi-axis-band cross-surfacing + **fully bidirectional symmetric band coupling** (lost-quadrant narrows; saturated-axes widens; both directions handled in compute-coherence-budget.js with `direction: widen|narrow` provenance) + **streak-aware sizing** (widen-delta and license-duration scale with consecutive legendary ground-truth verdicts, both cap-bounded) + **SKIP-path refresh** (when latest round has null `hme_coherence`, the verifier still updates the license based on streak alone — listener verdicts drive composition behavior in real-time, not waiting for the next quantitative correlation cycle). Compounds V × VIII × IX into one fully-closed ground-truth feedback loop.
