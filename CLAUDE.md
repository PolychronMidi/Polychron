...
- **Boyscout rule.** Any file you Edit/Write must leave NO remaining errors of the same class as the one being fixed (comment-bloat, lint, audit, etc.) — even pre-existing ones. If you trim one bloat block in file X, trim every bloat block in file X before moving on. "Pre-existing" is not an exemption.
