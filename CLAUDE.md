...
- **Never burst-probe third-party paid APIs with the user's credentials.** Any code path that spends the user's subscription credits (OAuth-routed Anthropic calls via the proxy, paid-tier provider keys, etc.) is single-shot-by-default. Sequential probes against the same account trip abuse detection and can get the account blocked. Before hitting a paid endpoint for TESTING: (a) look up the answer in docs or the codebase first; (b) if a live call is genuinely required, use the cheapest viable model (Haiku, not Opus) to prove the mechanics; (c) separate calls by at least a few seconds; (d) never put paid endpoints inside a loop. This rule survives across sessions because LIFESAVER cannot recover a banned account.
